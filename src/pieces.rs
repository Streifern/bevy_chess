...
impl Piece {
    // returns the possible positions that are available
    pub fn is_move_valid(&self, new_position: (u8, u8), pieces: Vec<Piece>) -> bool {
        // if there is a piece of the same color in the new position, there will be no move.
        if color_of_piece_in_square(new_position, &pieces) == Some(self.color) {
            return false;
        }
        // check if the move is a valid move for the piece type
            ...
            PieceType::Pawn => {
                if self.color == PieceColor::White {
                    ...
                    if new_position.0 as i8 - self.x as i8 == 1 && (self.y == new_position.1) {
                        if color_of_piece_in_square(new_position, &pieces).is_none() {
                            return true;
                        }
                    }
                    ...
                    if new_position.0 as i8 - self.x as i8 == 1
                        && (self.y as i8 - new_position.1 as i8).abs() == 1
                    {
                        if color_of_piece_in_square(new_position, &pieces) == Some(PieceColor::Black) {
                            return true;
                        }
                    }
                } else {
                    ...
                    if new_position.0 as i8 - self.x as i8 == -1
                        && (self.y as i8 - new_position.1 as i8).abs() == 1
                    {
                        if color_of_piece_in_square(new_position, &pieces) == Some(PieceColor::White) {
                            return true;
                        }
                    }
                }
...
fn spawn_knight(
    ...
    .spawn_bundle(PbrBundle {
        transform: { 
            ...
            // rotating the black knights, so they face the opponent
            if piece_color == PieceColor::Black {
                transform.rotate_around(Vec3::new(
                    position.0 as f32, 
                    0., 
                    position.1 as f32,
                ), Quat::from_rotation_y(3.141593));
            }
            transform
        },
        ..Default::default()
...
fn spawn_pawn(
    ...
    .insert(Name::new("Pawn"));
}


// return None if square is empty, return Some() wuth the color of the piece if not.
fn color_of_piece_in_square( position: (u8, u8), pieces: &Vec<Piece>) -> Option<PieceColor> {
    for piece in pieces {
...
fn is_path_empty(
    ...
    pieces: &Vec<Piece>) -> bool {
    // same column
    if begin.0 == end.0 {
        for piece in pieces {
            if piece.x == begin.1 && ((piece.y > begin.1 && piece.x < end.1) || (piece.y > end.1 && piece.y < begin.1)) {
                return false; 
            }
        }
    }
    ...
    if x_diff == y_diff {
        for i in 1..x_diff {
            ...
            if color_of_piece_in_square(pos, pieces).is_some() {
                return false;
            }
        }
    }
